        ...
        <nav className="bg-white text-bam-navy border-b border-gray-100 shadow-sm sticky top-0 z-50">
            <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-8 h-20">
                ...
                <Link to="/" className="flex flex-col items-center shrink-0">
                    <span className="text-3xl lg:text-4xl font-black text-bam-navy leading-none tracking-tighter uppercase">{t('common.app_name') || 'Christian Focus'}</span>
                ...
                >
                    ...
                    <input
                        type="text"
                        placeholder={t('common.search_placeholder') || 'Search our spiritual collection...'}
                        value={searchTerm}
                ...
                <div className="flex items-center space-x-6 shrink-0">
                    ...
                    {isAuthenticated ? (
                        <div className="flex items-center space-x-6">
                            ...
                            >
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest mt-0.5">
                                    {user?.role === 'ADMIN' ? t('nav.admin') || 'Admin' : user?.name?.split(' ')[0]}
                                </span>
                            </Link>
                        ...
                        <Link to="/login" className="flex flex-col items-center text-bam-navy hover:text-bam-red transition-colors group">
                            <User size={26} strokeWidth={1.5} />
                            <span className="text-[10px] font-bold uppercase tracking-widest mt-0.5 group-hover:underline underline-offset-2 decoration-2 decoration-bam-red">{t('nav.sign_in') || 'Sign In'}</span>
                        </Link>
                    )}
                    ...
                    <Link to="/saved" className="flex flex-col items-center text-bam-navy hover:text-bam-red transition-colors group">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest mt-0.5 group-hover:underline underline-offset-2 decoration-2 decoration-bam-red">{t('nav.saved') || 'Saved'}</span>
                    </Link>
